...
support builds 8, 16, 32 and 64 bit word sizes. Base station and drone
do not necessarily need the same word size. Public and private keys are
fixed and will be made available when the application is built.
...
2) Run base station or drone: make run_<entity>
   ...
   -> Application will ask for IP address. Enter the IP address of the
      OTHER entity. E.g. if you run base station, provide IP address of
      drone.
...
4) Operations for base station (only valid when session is active):
   ...
   -> f: start video feed. Because video feed is unavailable for UNIX
         systems, we transfer an image (./feed.jpg) instead. You can
         transfer your own image by replacing the ./feed.jpg file. The base
         station stores the received file in ./rec_feed.jpg. Note that the
         received file may contains errors because no retransmissions are
         done.
